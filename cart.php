...
<?php
      ...
      if($view == 'maybe') {  
        ...
        if(htmlentities($_GET['action']) == 'removed') {
            $removephoto = mysql_query("DELETE FROM usersmaybe WHERE imageid = '$imageid' AND (emailaddress = '$email' OR ip_address = '$ip')");
        }
        
        //Move photo to cart
        if(htmlentities($_GET['action']) == 'cart') {
            $deletequery = mysql_query("DELETE FROM usersmaybe WHERE (emailaddress = '$email' OR ip_address = '$ip') AND imageid = '$imageid'");
...
if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
    ...
    if($numpurchased > 0) {
        ...
        if(htmlentities($_GET['action']) == 'removed') {
            $deletequery = mysql_query("DELETE FROM userscart WHERE (emailaddress = '$email' OR ip_address = '$ip') AND imageid = '$imageid'");
        }
