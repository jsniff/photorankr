...
$useremail = $_GET['user'];
$lastpicture = $_GET['lastPicture'];

echo'<script type="text/javascript" src="js/jquery.wookmark.js"></script>';

if($_GET['lastPicture']) {
	$activityquery = mysql_query("SELECT * FROM newsfeed WHERE hide <> 1 AND id < '$lastpicture' AND (emailaddress = '$useremail' OR owner = '$useremail') AND type IN ('follow','comment','fave','photo') ORDER BY id DESC LIMIT 13");
