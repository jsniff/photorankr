...
&& $_SESSION['loggedin']==1)
{
	if ($_FILES["file"]["error"] > 0)
		...
		$tag4=mysql_real_escape_string($_POST['tag4']);	
        	...
        	$singlestyletags = mysql_real_escape_string($_POST['singlestyletags']);	
        	$singlecategorytags = mysql_real_escape_string($_POST['singlecategorytags']);
		...
		if (!$name | !$location | !$camera | $price < 0) 
		{
        ...
        $exif = exif_read_data($source, 0, true);
		...
		$insertquery="INSERT INTO photos (source, caption, emailaddress, tag, time, price, location, country, tag1, tag2, tag3, tag4, camera, focallength, shutterspeed, aperture, iso, software, lens, filter, about, copyright, sets, maintags, settags, set_id, singlestyletags, singlecategorytags)
		VALUES ('$target', '$name', '$email', '$tag', '$currenttime', '$price', '$location', '$country', '$tag1', '$tag2', '$tag3', '$tag4', '$camera', '$focallength', '$shutterspeed', '$aperture', '$iso', '$software', '$lens', '$filter', '$about', '$copyright', '$addtoset', '$maintags2','$settags2','$set_id','$singlestyletags2','$singlecategorytags2')";
