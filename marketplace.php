...
if($category == 'personal') {
    ...
    elseif($_SESSION['loggedin'] == 1) {
        ...
        $followcheckquery = mysql_query("SELECT following FROM userinfo WHERE emailaddress = '$email'");
        $followcheck = mysql_result($followcheckquery,0,'following');
                        
        $query = mysql_query("SELECT * FROM photos WHERE emailaddress IN ($followcheck) ORDER BY id DESC LIMIT 0,60");
        $numresults = mysql_num_rows($query);
...
for($iii=0; $iii < $numresults; $iii++) {
    ...
    echo'<div class="fPic"  id="',$imageid,'" style="width:180px;height:200px;overflow:hidden;float:left;border-top:1px solid #ccc;"><br /><a href="fullsizemarket.php?imageid=',$imageid,'"><div style="width:',$heightnew,'px;"><img id="popover',$iii,'" rel="popover" data-content="<span style=font-family:helvetica;font-weight:200;font-size:13px;>Rating: ',$rating,'<br />Full Resolution: ',$fullres,'<br />Photographer: ',$fullname,'</span><br /><br /><img src=',$imagebig2[$iii],' height=',$widthmed,'px width=',$heightmed,'px  />" data-original-title="',$title,'" onmousedown="return false" oncontextmenu="return false;" class="phototitletest" style="margin-top:20px;clear:right;float:bottom;margin:auto;" src="',$imagebig2[$iii],'" height="',$widthnew,'px" width="',$heightnew,'px" /></a>
