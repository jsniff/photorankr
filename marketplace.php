<?php
 ...
 if($_GET['action'] == "searchandfind") { //if they tried to sign up from index3.php
        $lowprice = addslashes($_POST['column']);
                $highprice = addslashes($_POST['column2']);

                $lowreputation = addslashes($_POST['reputationcolumn']);
                $highreputation = addslashes($_POST['reputationcolumn2']);

                $lowranking = addslashes($_POST['rankingcolumn']);
                $highranking = addslashes($_POST['rankingcolumn2']);


                        $lowdownload = addslashes($_POST['downloadcolumn']);
                $highdownload = addslashes($_POST['downloadcolumn2']);
...
"></b></a>
        ...
        ?>
					...
					<li class="navbartext"><form class="navbar-search" action="Marketplace.php" method="get">
...
//modify this to only include price range
...
//$price=mysql_result($result, 0, "price");
      ...
      $pricephoto=mysql_result($photoresult, $iii, "price");
        ...
        settype($pricephoto, "integer");



//This is by reputation level

    $toprankedphotos = "SELECT * FROM photos WHERE emailaddress = '$thisemail' ORDER BY points DESC";
...
$prevpoints=$photoarray['points'];
$prevvotes=$photoarray['votes'];
$ranking = $prevpoints/$prevvotes;
...
if($pricephoto > $lowprice && $pricephoto < $highprice && $ultimatereputation > $lowreputation && $ultimatereputation < $highreputation && $ranking > $lowranking && $ranking < $highranking) {
  ...
  //This is coding for reputation level
