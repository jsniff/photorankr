...
//Search Photos
$searchquery = mysql_query("SELECT source,caption,points,votes,faves,id FROM photos WHERE concat(id,caption,location,tag1,tag2,tag3,tag4) LIKE '%$searchword%' ORDER BY views DESC");
$numresults = mysql_num_rows($searchquery);

//Search Users
$searchterm = explode(" ",$searchword);
$searchusersquery  =  mysql_query("SELECT * FROM userinfo WHERE firstname LIKE '%".$searchterm[0]."%' AND lastname  LIKE '%".$searchterm[1]."%' OR lastname LIKE '%".$searchterm[0]."%' ORDER BY reputation DESC");
$numuserresults = mysql_num_rows($searchusersquery);


echo'<div id="searchDiv" class="uiScrollableAreaTrack invisible_elem" style="width:320px;height:500px;border:1px solid #666;-webkit-border-radius: 2px;-moz-border-radius: 2px;border-radius: 2px;background-color:rgb(250,250,250);position:absolute;top:42px;overflow:hidden;overflow-y:scroll;">

<div style="border-bottom:1px solid #aaa;text-align:left;font-family:\'helvetica neue\', helvetica, arial;font-size:17px;font-weight:300;padding:5px;">Photos  &mdash; ',$numresults,' Results</div>';
...
echo'<div style="border-bottom:1px solid #aaa;text-align:left;font-family:\'helvetica neue\', helvetica, arial;font-size:17px;font-weight:300;padding:5px;">Members &mdash; ',$numuserresults,' Results</div>';
