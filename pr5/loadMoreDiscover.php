...
if($_GET['lastPicture']) {
        ...
        $keyword = htmlentities($_GET['photos']);
        echo $keyword .'test';
        $prevviews = htmlentities($_GET['views']);
             echo $prevviews .'test';
             
        $query = "SELECT * FROM photos WHERE concat(caption,location,camera,tag1,tag2,tag3,tag4) LIKE '%$keyword%' AND views < '$prevviews' ORDER BY views DESC LIMIT 0,16";
        echo $query;
