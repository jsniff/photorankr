...
$newid = $currentid;
$getprevimages = mysql_query("SELECT id,source,caption,price,about FROM photos WHERE emailaddress = '$email' AND id < $newid ORDER BY id DESC LIMIT 0,1");

//If first image, take them to last image
if(mysql_num_rows($getprevimages) == 0) {
    $getprevimages = mysql_query("SELECT id,source,caption,price,about FROM photos WHERE emailaddress = '$email' ORDER BY id DESC LIMIT 0,1");
}
...
$jsondata = json_encode(array("nextimg" => $nextimg, "nextimgid" => $nextimgid, "previmgid" => $previmgid, "caption" => $caption, "price" => $price, "about" => $about));
echo $jsondata;
