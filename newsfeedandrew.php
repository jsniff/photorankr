...
$whosonline = mysql_query("SELECT * FROM userinfo WHERE loggedin = 1 AND emailaddress IN ($followlist) ORDER BY firstname ASC");
$numusers = mysql_num_rows($whosonline);

if(!$numusers) {
$numusers = 0;
}

echo'<div id="hideshow" style="margin-top:60px;font-weight:300;font-size:15px;color:#333;text-align:center;padding-top:7px;">',$numusers,' online (following)</div>';

echo'<div id="whosonline">';

for($iii=0;$iii<$numusers;$iii++) {
   
    $profilepic =mysql_result($whosonline,$iii,'profilepic');
    if($profilepic == '') { 
        $profilepic = 'graphics/noprofilepic.png';
    }
    $userid = mysql_result($whosonline,$iii,'user_id');
    $name = mysql_result($whosonline,$iii,'firstname') . " " . mysql_result($whosonline,$iii,'lastname');
    $emailaddress = mysql_result($whosonline,$iii,'emailaddress');
    
    echo'<a style="text-decoration:none;color:#333;" href="viewprofile.php?u=',$userid,'"><div id="chatbox" style="padding:3px;font-size:13px;color:#333;font-weight:300;font-family:helvetica,arial;"><img src="',$profilepic,'" style="width:45px;" /> ',$name,'</div></a>';
...
$newsfeedquery = "SELECT * FROM newsfeed WHERE (owner IN ($followlist) OR emailaddress IN ($followlist)) AND emailaddress NOT IN ('$email','') AND type NOT IN ('message','reply') ORDER BY id DESC LIMIT 0,20";
$newsfeedresult = mysql_query($newsfeedquery);
$newsfeedresult2 = mysql_query($newsfeedquery);
$newsfeedresultnext = mysql_query($newsfeedquery);
$maxwidth = 400;
...
$previousnewstype = ""; //stores what the previous item type in the newsfeed was, prevents multiple similar items in a row in newsfeed
$previousnewsuser = ""; //stores who the previous user that appeared in the newsfeed was
//$temprow = mysql_fetch_array($newsfeedresult);
//echo 'BOOYA';
$counter = 0;
for($iii=0; $iii <= 19; $iii++) {
    
		//$newsrownum = mysql_num_rows($newsfeedresult);
		mysql_data_seek($newsfeedresult, $counter);
		$newsrow = mysql_fetch_array($newsfeedresult);
		
		
		$id = $newsrow['id'];
		$pos = strpos($idlist,$id);
		$newsemail = $newsrow2['emailaddress'];    
		$owner = $newsrow['owner'];
		$time = $newsrow['time'];
	...
	if($view == '') {
		...
		elseif ($type == "fave" || $type == "discoverfave") {
			...
			//check first to see if it's the same user making a favorite next
			$newsrow = mysql_fetch_array($newsfeedresult);
			$nextcommentersquery = mysql_query("SELECT user_id,profilepic FROM userinfo WHERE emailaddress = '$commenteremail'");
			$nextcommenterid = mysql_result($commentersquery,0,'user_id');
			$nexttype = $newsrow['type'];
			if($nexttype == "fave" && $nextcommenterid == $commenterid){
				while($nexttype == "fave" && $nextcommenterid == $commenterid){
					if($type == "fave") {
						$phrase = $fullname . " favorited " . '"' . $caption . '"' . " by " . $ownerfull . " counter: " . $iii . " previous newsfeed type: " . $previousnewstype . " IDHERE: " . $commenterid . " next commenter id: " . $nextcommenterid;
					}
					elseif($type == "discoverfave") {
						$phrase = $fullname . " discovered " . '"' . $caption . '"' . " by " . $ownerfull;
					}
					$counter++; //counts how many time the next item in news feed is the same, used for data seek afterwards to skip those entries
				...
				if($type == "fave") {
					$phrase = $fullname . " favorited " . '"' . $caption . '"' . " by " . $ownerfull . " counter: " . $iii . " previous newsfeed type: " . $previousnewstype . " ID: " . $commenterid;
				}
				elseif($type == "discoverfave") {
					$phrase = $fullname . " discovered " . '"' . $caption . '"' . " by " . $ownerfull;
				}
			...
			//Faves List Model -- favorited by: etc..
	...
	$previousnewstype = $type;
	$counter++;
} //end for loop to gather news feed/views
